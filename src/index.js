  ...
  jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
    if (err) {
      ...
      dynamodb.scan(params, (err, data) => {
        if (err) {
          console.log(err)
          res.status(500).json({ message: 'We exprected technical details...' })
        } else {
...
const getSearchDetails = async (data, index = 0) => {
  ...
  if (data[index]) {
    await axios.get(`/searches/${data[index].id}/details`).then(async res => {
      const companies = res.data?.content.data.hits.slice(0, 1).map(company => {
...
app.get('/', (req, res) => {
  const parsedData = parseData(
    `${__dirname}/../files/success-file-example.xlsx`
  )
  res.json(parsedData)
})
